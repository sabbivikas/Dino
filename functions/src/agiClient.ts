...
export async function runAgentTask(params: RunParams): Promise<RunResult> {
  ...
  try {
    ...
    for await (const msg of transport.streamEvents(handle, controller.signal)) {
      all.push(msg);

      if (msg.type === "QUESTION") { outcome = "waiting"; question = msg.content; break; }
      if (msg.type === "ERROR") { outcome = "error"; errored = true; break; }
      if (msg.type === "DONE") { outcome = "finished"; break; }
      ...
      if (decision.kill) { killReason = decision.reason; outcome = "killed"; break; }
    }
  } catch (err) {
    if (timedOut) {
      outcome = "killed";
  ...
  const done = lastOfType(all, "DONE");
  const lastContent = done?.content ?? (all.length ? all[all.length - 1].content : "");
