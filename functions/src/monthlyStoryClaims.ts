...
const DEFINITIONS: Readonly<Record<MonthlyStoryClaimKey, ClaimDefinition>> = Object.freeze({
  ...
  recommendationOpened: claim("recommendationOpened", "recommendation",
    ["i sent you something when things felt heavy. i hope it gave you a little break"]),
  ...
  protectPersonalTime: claim("protectPersonalTime", "suggestion", ["protect a little more time where work is actually over"]),
  seekConnection: claim("seekConnection", "suggestion", ["make room for an easy moment with someone you trust"]),
  continueHelpfulPractice: claim("continueHelpfulPractice", "suggestion", ["keep one quiet practice easy to return to"]),
  makeSpaceForProjects: claim("makeSpaceForProjects", "suggestion", ["save a small pocket of time for your own ideas"]),
});
